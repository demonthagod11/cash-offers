...
@router.post("/receive")
def receive_payment(payment: ReceivePayment, db: Session = Depends(get_db)):
    ...
    logger.info(f"Payment received from {payment.from_account} to {payment.to_account}, Amount: {payment.amount}")
    return {"message": "Payment received successfully", "payment": db_payment}